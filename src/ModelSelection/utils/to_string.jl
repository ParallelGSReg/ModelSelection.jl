...
function sprintf_center(text::Union{String, Symbol, Int32, Int64, Float32, Float64}; len::Int = LINE_LENGTH, new_line::Bool = false)
...
function sprintf_variable(value::String; left_padding::Int = 0, right_padding::Int = 0)
    return sprintf_variable_base(value, left_padding = left_padding, right_padding = right_padding)
end

function sprintf_variable(value::Symbol; left_padding::Int = 0, right_padding::Int = 0)
    return sprintf_variable_base(string(value), left_padding = left_padding, right_padding = right_padding)
end

function sprintf_variable(value::Union{Float32, Float64}; left_padding::Int = 0, right_padding::Int = 0)
    return sprintf_variable_base(@sprintf("%-10f", value), left_padding = left_padding, right_padding = right_padding)
end

function sprintf_variable(value::Union{Int32, Int64}; left_padding::Int = 0, right_padding::Int = 0)
    return sprintf_variable_base(@sprintf("%-10d", value), left_padding = left_padding, right_padding = right_padding)
end

function sprintf_label_values(text::Union{String, Symbol}, values::Union{String, Array{String}, Int, Array{Int}, Float32, Array{Float32}, Float64, Array{Float64}}; len::Int = HALF_LINE_LENGTH, new_line::Bool = false)
...
function sprintf_depvar_block(data::ModelSelection.ModelSelectionData; len = HALF_LINE_LENGTH)
...
function sprintf_covvar(varname, datanames_index, result, result_data)
    ...
    if result.ttest
        values = vcat(values, [result_data[datanames_index[Symbol(string(varname, "_bstd"))]], result_data[datanames_index[Symbol(string(varname, "_t"))]]])
    end
    return sprintf_label_values(varname, values, new_line=true)
end


function sprintf_covvars_block(covvars_title::String, datanames_index::Dict{Symbol, Int64}, expvars::Vector{Symbol}, data, result, result_data)
...
function sprintf_summary(datanames_index, result, result_data; summary_variables = nothing, criteria_variables = nothing)
    ...
    if summary_variables !== nothing
        ...
        for variable in variables
            ...
            out *= ModelSelection.sprintf_label_values(variable["verbose_title"], result_data[datanames_index[variable["varname"]]], new_line=true)
    ...
    if criteria_variables !== nothing
        for (variable, value) in criteria_variables
            ...
            out *= ModelSelection.sprintf_label_values(value["verbose_title"], result_data[datanames_index[variable]], new_line=true)
