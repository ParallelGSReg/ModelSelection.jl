...
function create_figures(data::ModelSelection.ModelSelectionData, destfolder::String)
    ...
    for (i, expvar) in enumerate(expvars2)
        bcol = ModelSelection.get_column_index(
            Symbol("$(expvar)_b"),
            data.results[1].datanames,
        )
        tcol = ModelSelection.get_column_index(
            Symbol("$(expvar)_t"),
            data.results[1].datanames,
        )

        r2col = ModelSelection.get_column_index(:r2adj, data.results[1].datanames)
        x = data.results[1].data[
            findall(x -> !isnan(x), data.results[1].data[:, bcol]),
            bcol,
        ]
        if tcol !== nothing
            y = data.results[1].data[
                findall(x -> !isnan(x), data.results[1].data[:, tcol]),
                tcol,
            ]
        ...
        criteria_with = data.results[1].data[
            findall(x -> !isnan(x), data.results[1].data[:, bcol]),
            r2col,
        ]
        criteria_without = data.results[1].data[
            findall(x -> isnan(x), data.results[1].data[:, bcol]),
            r2col,
        ]
        ...
        p1 = plot(
            range(min(criteria_with...), stop = max(criteria_with...), length = 150),
            z -> pdf(uniden_with, z),
        ) # FIXME: This should not be working
        p1 = plot!(
            range(min(criteria_without...), stop = max(criteria_without...), length = 150),
            z -> pdf(uniden_without, z),
        ) # FIXME: This should not be working
        ...
        p2 = violin(
            ["Including $(expvar)" "Excluding $(expvar)"],
            [criteria_with, criteria_without],
            leg = false,
            marker = (0.1, stroke(0)),
            alpha = 0.50,
            color = :blues,
    ...
    bar(
        a[:, 1],
        orientation = :h,
        yticks = (1:size(a, 1), labels),
        legend = false,
        color = :blues,
        xlabel = "Average impact of each variable on the Adj. R2",
    )
...
function latex(
    ...
    if size(data.results, 1) > 0
        ...
        for i = 1:size(data.results, 1)
            latex!(dict, data, originaldata, data.results[i])
        end
        create_workspace(tempfolder)
        if data.results[1].ttest
            dict[string(ModelSelection.AllSubsetRegression.ALLSUBSETREGRESSION_EXTRAKEY)]["intelligent_text"] =
...
function latex!(
    ...
    preprocessing_dict["equation"] =
        join(map(x -> "$x", filter(x -> x != :_cons, preprocessing_dict["datanames"])), " ")
    preprocessing_dict["datanames"] =
        string("[:", join(preprocessing_dict["datanames"], ", :"), "]")
    ...
    if "seasonaladjustment" in keys(preprocessing_dict) &&
       preprocessing_dict["seasonaladjustment"] !== nothing
        preprocessing_dict["seasonaladjustment"] =
            get_array_details(preprocessing_dict["seasonaladjustment"])
    end
    ...
    if FeatureExtraction.FEATUREEXTRACTION_EXTRAKEY in keys(data.extras)
        ...
        featureextraction_dict =
            process_dict(data.extras[FeatureExtraction.FEATUREEXTRACTION_EXTRAKEY])
        ...
        if "fe_log" in keys(featureextraction_dict)
            featureextraction_dict["fe_log"] =
                replace(string(featureextraction_dict["fe_log"]), "Symbol" => "")
        end
        if "fe_inv" in keys(featureextraction_dict)
            featureextraction_dict["fe_inv"] =
                replace(string(featureextraction_dict["fe_inv"]), "Symbol" => "")
        end
        if "fe_sqr" in keys(featureextraction_dict)
            featureextraction_dict["fe_sqr"] =
                replace(string(featureextraction_dict["fe_sqr"]), "Symbol" => "")
        end
    ...
    if PreliminarySelection.PRELIMINARYSELECTION_EXTRAKEY in keys(data.extras)
        dict[string(PreliminarySelection.PRELIMINARYSELECTION_EXTRAKEY)] =
            process_dict(data.extras[PreliminarySelection.PRELIMINARYSELECTION_EXTRAKEY])

        betas =
            dict[string(PreliminarySelection.PRELIMINARYSELECTION_EXTRAKEY)]["lassobetas"]
        betas_dict = []
        for (i, beta) in enumerate(filter(j -> j != 0, betas))
            if (beta != 0)
                push!(
                    betas_dict,
                    Dict(
                        "name" => replace(string(data.expvars[i]), "_" => "\\_"),
                        "coef" => @sprintf("%.3f", beta),
                    ),
                )
            end
        end
        ...
        dict[string(PreliminarySelection.PRELIMINARYSELECTION_EXTRAKEY)]["expvssars"] =
            betas_dict
    end

    # Output
    if Output.OUTPUT_EXTRAKEY in keys(data.extras)
        dict[string(Output.OUTPUT_EXTRAKEY)] =
            process_dict(data.extras[Output.OUTPUT_EXTRAKEY])
    end
...
function latex!(
    ...
    if ModelSelection.AllSubsetRegression.ALLSUBSETREGRESSION_EXTRAKEY in keys(data.extras)
        dict[string(ModelSelection.AllSubsetRegression.ALLSUBSETREGRESSION_EXTRAKEY)] =
            process_dict(
                data.extras[ModelSelection.AllSubsetRegression.ALLSUBSETREGRESSION_EXTRAKEY],
            )
        ...
        for var in data.expvars
            ...
            if (
                var in data.expvars[cols] &&
                !isnan(result.bestresult_data[datanames_index[Symbol("$(var)_b")]])
            )
                d["best"] = Dict()
                d["best"]["b"] = @sprintf(
                    "%.3f",
                    result.bestresult_data[datanames_index[Symbol("$(var)_b")]]
                )
                if (result.ttest)
                    d["best"]["ttest"] = true
                    d["best"]["bstd"] = @sprintf(
                        "%.3f",
                        result.bestresult_data[datanames_index[Symbol("$(var)_bstd")]]
                    )
                    t = result.bestresult_data[datanames_index[Symbol("$(var)_t")]]
                    prob_t = pdf(
                        TDist(data.nobs - length(data.expvars) - (
                                if data.intercept
                                    1
                                else
                                    0
            ...
            if (
                result.modelavg &&
                !isnan(result.modelavg_data[datanames_index[Symbol("$(var)_b")]])
            )
                d_bestmodel["modelavg"] = true
                d["avg"] = Dict()
                d["avg"]["b"] = @sprintf(
                    "%.3f",
                    result.modelavg_data[datanames_index[Symbol("$(var)_b")]]
                )
                if (result.ttest)
                    d["avg"]["ttest"] = true
                    d["avg"]["bstd"] = @sprintf(
                        "%.3f",
                        result.modelavg_data[datanames_index[Symbol("$(var)_bstd")]]
                    )
        ...
        expvars_dict = map(
            var -> begin
                t = result.data[
                    :,
                    ModelSelection.get_column_index(Symbol("$(var)_t"), result.datanames),
                ]
                b = result.data[
                    :,
                    ModelSelection.get_column_index(Symbol("$(var)_b"), result.datanames),
                ]
                ...
                Dict(
                    ...
                    "sigshare" => @sprintf(
                        "%.3f",
                        size(filter(p -> p < 0.1, prob_t), 1) / size(b_nnan, 1)
                    ),
        ...
        dict[string(ModelSelection.AllSubsetRegression.ALLSUBSETREGRESSION_EXTRAKEY)]["expvars"] =
            expvars_dict
        ...
        dict[string(ModelSelection.AllSubsetRegression.ALLSUBSETREGRESSION_EXTRAKEY)]["bestmodel"] =
            d_bestmodel

        if dict[string(ModelSelection.AllSubsetRegression.ALLSUBSETREGRESSION_EXTRAKEY)]["criteria"] !=
           nothing
            dict[string(ModelSelection.AllSubsetRegression.ALLSUBSETREGRESSION_EXTRAKEY)]["criteria"] =
                string(
                    ...
                    join(
                        dict[string(
                            ModelSelection.AllSubsetRegression.ALLSUBSETREGRESSION_EXTRAKEY,
                        )]["criteria"],
                        ", :",
                    ),
                    "]",
        ...
        if dict[string(ModelSelection.AllSubsetRegression.ALLSUBSETREGRESSION_EXTRAKEY)]["residualtest"] !=
           false
            if "time" in keys(
                dict[string(
                    ModelSelection.AllSubsetRegression.ALLSUBSETREGRESSION_EXTRAKEY,
               ...
               dict[string(
                ...
                dict[string(
                    ModelSelection.AllSubsetRegression.ALLSUBSETREGRESSION_EXTRAKEY,
                )]["residualtestfortex2"] = true
            else
                dict[string(
                    ModelSelection.AllSubsetRegression.ALLSUBSETREGRESSION_EXTRAKEY,
                )]["residualtestfortex"] = true
            end
        end
    end
    return dict
...
function latex!(
    ...
    if ModelSelection.CrossValidation.CROSSVALIDATION_EXTRAKEY in keys(data.extras)
        dict[string(ModelSelection.CrossValidation.CROSSVALIDATION_EXTRAKEY)] =
            process_dict(
                data.extras[ModelSelection.CrossValidation.CROSSVALIDATION_EXTRAKEY],
            )
        ...
        for var in data.expvars
            ...
            if (!isnan(result.average_data[datanames_index[Symbol("$(var)_b")]]))
                ...
                d["mean"]["b"] = @sprintf(
                    "%.6f",
                    result.average_data[datanames_index[Symbol("$(var)_b")]]
                )
                if (result.ttest)
                    d["mean"]["ttest"] = true
                    d["mean"]["bstd"] = @sprintf(
                        "%.6f",
                        result.average_data[datanames_index[Symbol("$(var)_bstd")]]
                    )
                    t =
                        result.average_data[datanames_index[Symbol("$(var)_b")]] /
                        result.average_data[datanames_index[Symbol("$(var)_bstd")]]
                ...
                d["median"]["b"] = @sprintf(
                    "%.6f",
                    result.median_data[datanames_index[Symbol("$(var)_b")]]
                )
                if (result.ttest)
                    d["median"]["ttest"] = true
                    d["median"]["bstd"] = @sprintf(
                        "%.6f",
                        result.median_data[datanames_index[Symbol("$(var)_bstd")]]
                    )
                    t =
                        result.median_data[datanames_index[Symbol("$(var)_b")]] /
                        result.median_data[datanames_index[Symbol("$(var)_bstd")]]
        ...
        dict[string(ModelSelection.CrossValidation.CROSSVALIDATION_EXTRAKEY)]["kfoldvars"] =
            d_bestmodel

        dict[string(ModelSelection.CrossValidation.CROSSVALIDATION_EXTRAKEY)]["outsample"] =
            Dict(
...
# TODO add description
dropnans(res, var) =
    res.results[findall(x -> !isnan(x), res.results[:, res.header[var]]), res.header[var]]
