...
function addextras(
    ...
    data.extras[ModelSelection.generate_extra_key(OUTPUT_EXTRAKEY, data.extras)] =
        Dict(:outputtype => outputtype, :filename => filename, :path => path)
    return data
end
...
function get_array_simple_details(arr::Array) # TODO: Better typing definition
    Dict(
        ((length(arr) == 1) ? "var" : "vars") => Dict("names" => map(string, arr)),
        ((length(arr) == 1) ? "vars" : "var") => false,
    )
end
