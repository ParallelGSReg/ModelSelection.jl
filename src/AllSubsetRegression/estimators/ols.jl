...
function execute!(data::ModelSelection.ModelSelectionData, result::AllSubsetRegressionResult)
	...
	if :cp in result.criteria
		result.data[:, datanames_index[:cp]] =
			(result.data[:, datanames_index[:nobs]] .- maximum(result.data[:, datanames_index[:ncoef]]) .- 2) .* (result.data[:, datanames_index[:rmse]] ./ minimum(result.data[:, datanames_index[:rmse]])) .-
			(result.data[:, datanames_index[:nobs]] .- 2 .* result.data[:, datanames_index[:ncoef]])
	end
...
function execute_row!(
	...
	for (index, selected_variable_index) in enumerate(selected_variables_index)
		...
		if ttest
			...
			result_data[order, datanames_index[Symbol(string(expvars[selected_variable_index], "_t"))]] =
	...
	if :aicc in criteria
		result_data[order, datanames_index[:aicc]] =
			aic + (2(result_data[order, datanames_index[:ncoef]] + 1) * (result_data[order, datanames_index[:ncoef]] + 2)) / (result_data[order, datanames_index[:nobs]] - (result_data[order, datanames_index[:ncoef]] + 1) - 1)
	end

	if :bic in criteria
		result_data[order, datanames_index[:bic]] =
			result_data[order, datanames_index[:nobs]] * log.(result_data[order, datanames_index[:rmse]]) + (result_data[order, datanames_index[:ncoef]] - 1) * log.(result_data[order, datanames_index[:nobs]]) + result_data[order, datanames_index[:nobs]] +
