...
function replace_string_to_nothing(data)
	for n in 1:size(data[1], 1)
		for m in 1:size(data[1], 2)
...
data_paneltime_without_missings_dataframe = CSV.read(paneltime_without_missing_filename, DataFrame)
data_paneltime_without_missings_tuple = replace_string_to_nothing(readdlm(paneltime_without_missing_filename, ',', header = true))
...
data_time_without_missings_tuple = replace_string_to_nothing(readdlm(time_without_missing_filename, ',', header = true))
...
data_paneltime_with_missings_dataframe = CSV.read(paneltime_with_missing_filename, DataFrame)

data_paneltime_with_missings_tuple = replace_string_to_nothing(readdlm(paneltime_with_missing_filename, ',', header = true))
...
data_paneltime_as_missings = replace_string_to_nothing(readdlm(paneltime_as_missing_filename, ',', header = true))
